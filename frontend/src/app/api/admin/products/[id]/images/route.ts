 ...
 */
async function validateAndSecureFile(file: File, index: number): Promise<{
  isValid: boolean;
  error?: string;
  tempPath?: string;
  suggestions?: string[];
  ...
  try {
    ...
    const tempPath = path.join(tempDir, `temp-${Date.now()}-${index}-${crypto.randomBytes(8).toString('hex')}${ext}`);
...
export async function POST(
  ...
  try {
    ...
    if (!checkRateLimit(clientIp)) {
      return NextResponse.json(
        { success: false, message: 'Limite de téléversement dépassée. Maximum 20 uploads par minute.' },
        { status: 429 }
      );
    }
    ...
    if (totalSize > MAX_TOTAL_SIZE) {
      ...
      return NextResponse.json(
        { success: false, message: `Taille totale dépassée (${ImageOptimizer.formatFileSize(totalSize)}). Maximum: ${ImageOptimizer.formatFileSize(MAX_TOTAL_SIZE)}` },
        { status: 400 }
      );
    }
    ...
    for (const validFile of validFiles) {
      try {
        ...
        errors.push(`${validFile.file.name}: Erreur lors de l'optimisation - ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
        await unlink(validFile.tempPath);
      }
    }
    ...
    console.log(`[AUDIT] Upload d'images pour le produit ${productId}: ${uploadedImages.length} images uploadées, ${errors.length} erreurs`);
...
export async function DELETE(
  ...
  try {
    ...
    if (!imageId) {
      return NextResponse.json(
        { success: false, message: 'ID image requis' },
        { status: 400 }
      );
    }
