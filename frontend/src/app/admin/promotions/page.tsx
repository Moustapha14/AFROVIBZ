...
export default function SuperAdminPromotionsPage() {
  ...
  const getTypeBadge = (type: string) => {
    const typeConfig = {
      ...
      'free_shipping': { color: 'bg-purple-100 text-purple-800', label: 'Livraison gratuite', icon: Gift },
    };
  ...
  const getStatusBadge = (promotion: Promotion) => {
    ...
    if (isExpired) {
      return <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Expirée</span>;
    }
    if (isNotStarted) {
      return <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">À venir</span>;
    }
    if (isLimitReached) {
      return <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800">Limite atteinte</span>;
    }
  ...
  const getPromotionStats = () => {
    ...
    return {
      ...
      active: promotions.filter(p => p.isActive && now >= p.startDate && now <= p.endDate && p.usedCount < p.usageLimit).length,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {filteredPromotions.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Aucune promotion ne correspond à vos critères de recherche.</p>
          </div>
        )}
      </div>
    </div>
