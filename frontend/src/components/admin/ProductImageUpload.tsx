...
export function ProductImageUpload({
  ...
  const calculateStats = () => {
    ...
    const totalOriginalSize = successImages.reduce(
      (sum, img) => sum + (img.stats?.originalSize || 0),
      0
    );
    const totalOptimizedSize = successImages.reduce(
      (sum, img) => sum + (img.stats?.optimizedSize || 0),
      0
    );
    const avgCompressionRatio =
      successImages.reduce((sum, img) => sum + (img.stats?.compressionRatio || 0), 0) /
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center space-x-2'>
          ...
          {images.filter(img => img.status === 'success').length > 1 && (
            <Button variant='outline' size='sm' onClick={() => setIsReordering(!isReordering)}>
            ...
            {images.map((image, index) => (
              ...
              >
                ...
                <div className='p-3'>
                  ...
                  {image.error && <p className='text-xs text-red-500 mt-1'>{image.error}</p>}
